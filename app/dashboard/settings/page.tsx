...
import { User, CreditCard, Users, ShieldAlert, ArrowLeft } from "lucide-react";
import Link from "next/link";

export default function SettingsPage() {

  const handleSave = () => {
    toast.success("Profile Updated", { description: "Your settings have been saved locally." });
  };

  const handleEnterpriseFeature = () => {
    toast.info("Enterprise Feature", { description: "Billing & Team management coming in v1.1 update." });
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="space-y-6">
        ...
        {/* Profile Section */}
        <div className="border rounded-lg p-6 bg-card/60 backdrop-blur-sm space-y-4"> {/* Added transparency */}
           ...
           <div className="grid gap-4">
             <div className="grid gap-2">
               ...
               <input type="text" defaultValue="Demo User" className="flex h-10 w-full rounded-md border border-input bg-background/50 px-3 py-2 text-sm ring-offset-background" />
             </div>
             <div className="grid gap-2">
               <label className="text-sm font-medium">Email Address</label>
               <input type="email" defaultValue="demo@example.com" disabled className="flex h-10 w-full rounded-md border border-input bg-muted/50 px-3 py-2 text-sm ring-offset-background opacity-50 cursor-not-allowed" />
             </div>
           </div>
        ...
        <div className="border rounded-lg p-6 bg-card/60 backdrop-blur-sm space-y-4 opacity-75">
